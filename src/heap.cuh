 ...
 *  Description:
 *    Make some modifications to campatible with the project.
 */
...
template <int NumWarps, int NumThreads, typename K, typename V, int NumWarpQ,
          bool SelectMax, typename Comp>
struct FinalBlockMerge {};

template <int NumThreads, typename K, typename V, int NumWarpQ, bool SelectMax,
          typename Comp>
struct FinalBlockMerge<1, NumThreads, K, V, NumWarpQ, SelectMax, Comp> {
...
// `SelectMax` true, produce largest values.
// `SelectMax` false, produce smallest values.
template <typename K, typename V, bool SelectMax, typename Comp, int NumWarpQ,
...
struct BlockHeap {
  ...
  __device__ inline BlockHeap(K initKVal, V initVVal, K* smemK, V* smemV, int k)
      : initK(initKVal),
        initV(initVVal),
        numVals(0),
        warpKTop(initKVal),
        ...
        sharedV(smemV),
        kMinus1(k - 1) {
    static_assert(isPowerOf2(ThreadsPerBlock), "threads must be a power-of-2");
...
#pragma unroll
  ...
  __device__ inline void addThreadQ(K k, V v) {
    if (SelectMax ? Comp::gt(k, warpKTop) : Comp::lt(k, warpKTop)) {
// Rotate right
#pragma unroll
...
#pragma unroll
  ...
  __device__ inline void mergeWarpQ() {
    ...
    warpSortAnyRegisters<K, V, NumThreadQ, !SelectMax, Comp>(threadK, threadV);
...
#pragma unroll
  ...
  // The k-th highest (SelectMax) or lowest (!SelectMax) element
  K warpKTop;
  ...
  // warpK[0] is highest (SelectMax) or lowest (!SelectMax)
  K* warpK;
  V* warpV;
...
template <typename K, typename V, bool SelectMax, typename Comp, int NumThreadQ,
          int ThreadsPerBlock>
struct BlockHeap<K, V, SelectMax, Comp, 1, NumThreadQ, ThreadsPerBlock> {
  static constexpr int kNumWarps = ThreadsPerBlock / kWarpSize;

  __device__ inline BlockHeap(K initK, V initV, K* smemK, V* smemV, int k)
      : sharedK(smemK), sharedV(smemV), threadK(initK), threadV(initV) {}

  __device__ inline void addThreadQ(K k, V v) {
    bool swap = SelectMax ? Comp::gt(k, threadK) : Comp::lt(k, threadK);
  ...
  __device__ inline void reduce() {
    ...
    if (SelectMax) {
      pair = warpReduce<Pair<K, V>, Max>(pair, Max<Pair<K, V>>());
    } else {
      pair = warpReduce<Pair<K, V>, Min>(pair, Min<Pair<K, V>>());
    }
...
#pragma unroll
  ...
  // threadK is lowest (SelectMax) or highest (!SelectMax)
  K threadK;
  V threadV;
...
struct WarpHeap {
  ...
  __device__ inline WarpHeap(K initKVal, V initVVal, int k)
      : initK(initKVal),
        initV(initVVal),
        numVals(0),
        warpKTop(initKVal),
...
template <typename K, typename V, bool SelectMax, typename Comp, int NumThreadQ,
          int ThreadsPerBlock>
struct WarpHeap<K, V, SelectMax, Comp, 1, NumThreadQ, ThreadsPerBlock> {
  static constexpr int kNumWarps = ThreadsPerBlock / kWarpSize;

  __device__ inline WarpHeap(K initK, V initV, int k)
