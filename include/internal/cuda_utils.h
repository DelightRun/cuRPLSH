...
inline int initCudaDevice(int device) {
  int device_count;
  checkCudaErrors(cudaGetDeviceCount(&device_count));

  if (device_count == 0) {
    fprintf(stderr,
            "initCudaDevice() CUDA error: "
  ...
  if (device > device_count - 1) {
    ...
    fprintf(stderr,
            ">> initCudaDevice (-device=%d) is not a valid"
            " GPU device. <<\n",
            device);
    fprintf(stderr, "\n");
    return -device;
  ...
  if (deviceProp.major < 1) {
    fprintf(stderr, "initCudaDevice(): GPU device does not support CUDA.\n");
    exit(EXIT_FAILURE);
  }

  checkCudaErrors(cudaSetDevice(device));
  printf("initCudaDevice() CUDA Device [%d]: \"%s\n", device, deviceProp.name);
...
inline int getCudaDevice(int argc, const char **argv) {
  ...
  if (checkCmdLineFlag(argc, argv, "device")) {
    device = getCmdLineArgumentInt(argc, argv, "device=");

    if (device < 0) {
      printf("Invalid command line parameter\n ");
      exit(EXIT_FAILURE);
    } else {
      device = initCudaDevice(device);
...
inline int getCurrentDevice() {
  ...
  runtime_assert(device != -1);

  return device;
}

inline void setCurrentDevice(int device) { checkCudaErrors(cudaSetDevice(device)); }

inline int getNumDevices() {
  int device_count = -1;
  checkCudaErrors(cudaGetDeviceCount(&device_count));
  runtime_assert(device_count != -1);
...
inline int getMaxThreadsPerBlock(int device) {
  return getDeviceProperties(device).maxThreadsPerBlock;
}

inline int getMaxThreadsPerBlockCurrentDevice() {
...
/// initializer_list versions
template <typename L1>
void streamWait(const L1 &l1, const std::initializer_list<cudaStream_t> &l2) {
  streamWaitBase(l1, l2);
}

template <typename L2>
void streamWait(const std::initializer_list<cudaStream_t> &l1, const L2 &l2) {
  streamWaitBase(l1, l2);
}

template <typename L1, typename L2>
inline void streamWait(const L1 &l1, const L2 &l2) {
  streamWaitBase(l1, l2);
}

#endif  // end of CUDA Helper Utils
